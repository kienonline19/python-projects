...
def update_color():
    ...
    color_box = tk.Label(input_frame, bg='#' + red_value +
                         green_value + blue_value, height=6, width=15)
    color_box.grid(row=1, column=3, columnspan=2,
                   padx=35, pady=10)

    # Display the tuple and hex value for the given color
    color_tuple.config(
        text=f"({red_slider.get()}),({green_slider.get()}),({blue_slider.get()})")
    color_hex.config(text='#' + red_value +
...
def get_red(slider_value):
    ...
    # Strip leading chars so only two remain
    red_value = hex(int(slider_value))[2:]
...
def get_green(slider_value):
    ...
    # Turn the slider value into an int and hex value.
    # Strip leading chars so only two remain
    green_value = hex(int(slider_value))[2:]
...
def get_blue(slider_value):
    ...
    # Turn the slider value into an int and hex value.
    # Strip leading chars so only two remain
    blue_value = hex(int(slider_value))[2:]
...
def store_color():
    ...
    red = str(red_slider.get())  # 12

    while len(red) < 3:
        red = '0' + red

    green = str(green_slider.get()).zfill(3)
    blue = str(blue_slider.get()).zfill(3)
    ...
    recall_button = tk.Button(output_frame, text="Recall Color", command=lambda: set_color(
        stored_red, stored_green, stored_blue))
    new_color_tuple = tk.Label(
        output_frame, text=f"({red}),({green}),({blue})")
    new_color_hex = tk.Label(output_frame, text='#' + red_value +
                             green_value + blue_value)
    new_color_black_box = tk.Label(output_frame, bg="black", width=3, height=1)
    new_color_box = tk.Label(output_frame, bg='#' + red_value +
                             green_value + blue_value, width=3, height=1)
    ...
    new_color_black_box.grid(row=stored_color.get(
    ), column=4, padx=20, pady=2, ipadx=5, ipady=5)
    new_color_box.grid(row=stored_color.get(), column=4)

    # Update the dict stored_colors with the new color tuple and hex values
    cur = stored_color.get()
    stored_colors[cur] = [new_color_tuple.cget(
        "text"), new_color_hex.cget("text")]

    # Move the radio buttons stored colors_ to the next value if available
    if cur < 5:
        stored_color.set(cur + 1)
...
def save_colors():
    ...
    with open(file_name, 'w') as f:
        ...
        for saved_entry in stored_colors.values():
            f.write(f"{saved_entry[0]}\n{saved_entry[1]}\n\n")


# Define GUI Layout
# Create frames
input_frame = tk.LabelFrame(root, padx=5, pady=5)
output_frame = tk.LabelFrame(root, padx=5, pady=5)
...
red_label = tk.Label(input_frame, text="R")
red_slider = tk.Scale(input_frame, from_=0, to=255, command=get_red)
red_button = tk.Button(input_frame, text="Red",
                       command=lambda: set_color(255, 0, 0))

green_label = tk.Label(input_frame, text="G")
green_slider = tk.Scale(input_frame, from_=0, to=255, command=get_green)
green_button = tk.Button(input_frame, text="Green",
                         command=lambda: set_color(0, 255, 0))

blue_label = tk.Label(input_frame, text="B")
blue_slider = tk.Scale(input_frame, from_=0, to=255, command=get_blue)
blue_button = tk.Button(input_frame, text="Blue",
                        command=lambda: set_color(0, 0, 255))

# Create buttons
yellow_button = tk.Button(input_frame, text="Yellow",
                          command=lambda: set_color(255, 255, 0))
cyan_button = tk.Button(input_frame, text="Cyan",
                        command=lambda: set_color(0, 255, 255))
magenta_button = tk.Button(input_frame, text="Magenta",
                           command=lambda: set_color(255, 0, 255))

# Create utility buttons
store_button = tk.Button(input_frame, text="Store Color", command=store_color)
save_button = tk.Button(input_frame, text="Save", command=save_colors)
quit_button = tk.Button(input_frame, text="Quit", command=root.destroy)

# Put labels, sliders, and buttons onto the frame...using ipadx with rgb buttons to define column width, then use sticky on others
...
color_box = tk.Label(input_frame, bg="black", width=15, height=6)
color_tuple = tk.Label(input_frame, text="(0),(0),(0)")
color_hex = tk.Label(input_frame, text="#000000")
...
# Create radio buttons to select stored colors and
# populate each row with placeholder values
for i in range(6):
    radio = tk.Radiobutton(output_frame, variable=stored_color, value=i)
    radio.grid(row=i, column=0, sticky="W")

    recall_button = tk.Button(
        output_frame, text="Recall Color", state=tk.DISABLED)
    new_color_tuple = tk.Label(output_frame, text="(255),(255),(255)")
    new_color_hex = tk.Label(output_frame, text="#ffffff")
    new_color_black_box = tk.Label(output_frame, bg="black", width=3, height=1)
    new_color_box = tk.Label(output_frame, bg="white", width=3, height=1)
    ...
    # .cget() return the values of a specific option. Store the text value of the tuple label and hex label
    stored_colors[stored_color.get()] = [new_color_tuple.cget(
        "text"), new_color_hex.cget("text")]

# Initialize the starting values for the color box display
red_value = green_value = blue_value = '00'
