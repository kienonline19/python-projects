...
def move(event):
    ...
    # If the curent object clicked has the "BALL" tag, we should allow it to be moved
    if "BALL" in main_canvas.gettags(CURRENT):
        ...
        # Attempt to not move the ball of the canvas. CURRENT[3] is y2 coord
...
def update_height():
    ...
    for i in range(1, 5):
        heights["height_%d" % i].config(
            text="Height: " + str(round(415 - main_canvas.coords(balls["ball_%d" % i])[3], 2)))


def step(t):
    """ Advance the ball one 'step' based on time slider value of t """
    global time
    ...
    for i in range(1, 5):
        ...
        velocities["v_%d" % i].delete(0, tk.END)
...
def graph():
    ...
    # Colors of the ball corresponds to colors of the graph
...
def reset():
    ...
    # Clear each ball...
    for i in range(1, 5):
        # Clear and set the velocity and acclerations
        velocities["v_%d" % i].delete(0, tk.END)
        velocities["v_%d" % i].insert(0, '0')
        accelerations["a_%d" % i].delete(0, tk.END)
        accelerations["a_%d" % i].insert(0, '0')

        # Reset ball to starting position
        main_canvas.coords(balls["ball_%d" % i], 45 +
                           (i-1)*100, 405, 55+(i-1)*100, 415)
        # Clear data
...
balls["ball_1"] = main_canvas.create_oval(
    45, 405, 55, 415, fill="red", tag="BALL")
balls["ball_2"] = main_canvas.create_oval(
    145, 405, 155, 415, fill="green", tag="BALL")
balls["ball_3"] = main_canvas.create_oval(
    245, 405, 255, 415, fill="blue", tag="BALL")
balls["ball_4"] = main_canvas.create_oval(
    345, 405, 355, 415, fill="yellow", tag="BALL")

# Input frame layout
# Row labels
tk.Label(input_frame, text='d').grid(row=0, column=0)
tk.Label(input_frame, text='vi').grid(row=1, column=0)
tk.Label(input_frame, text='a').grid(row=2, column=0, ipadx=22)
...
for i in range(1, 5):
    heights['height_%d' % i] = tk.Label(
        input_frame, text="Height: " + str(415 - main_canvas.coords(balls["ball_%d" % i])[3]))
...
# Time slider
t_slider = tk.Scale(input_frame, from_=0, to=1,
                    tickinterval=.1, resolution=.01, orient=tk.HORIZONTAL)
...
# Make each call 'dragable' in the vertical direction
