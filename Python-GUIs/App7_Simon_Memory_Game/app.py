...
def animate(button):
    ...
    root.after(time, lambda: button.config(state=tk.NORMAL))
...
def enable():
    ...
    white_button.config(state=tk.NORMAL)
    magenta_button.config(state=tk.NORMAL)
    cyan_button.config(state=tk.NORMAL)
    yellow_button.config(state=tk.NORMAL)
...
def check_round():
    ...
    # The player is correct, to change the label, update score, wait, then start the next round.
...
def disable():
    ...
    white_button.config(state=tk.DISABLED)
    magenta_button.config(state=tk.DISABLED)
    cyan_button.config(state=tk.DISABLED)
    yellow_button.config(state=tk.DISABLED)
...
start_button = tk.Button(info_frame, text="New Game",
                         font=game_font1, bg=game_color, command=enable)
score_label = tk.Label(info_frame, text="Score: " +
                       str(score), font=game_font1, bg=root_color)
...
white_button = tk.Button(game_frame, bg=white,
                         activebackground=white_light, borderwidth=3, state=tk.DISABLED, command=lambda: press(1))
magenta_button = tk.Button(game_frame, bg=magenta,
                           activebackground=magenta_light, borderwidth=3, state=tk.DISABLED, command=lambda: press(2))
cyan_button = tk.Button(game_frame, bg=cyan,
                        activebackground=cyan_light, borderwidth=3, state=tk.DISABLED, command=lambda: press(3))
yellow_button = tk.Button(game_frame, bg=yellow,
                          activebackground=yellow_light, borderwidth=3, state=tk.DISABLED, command=lambda: press(4))

white_button.grid(row=0, column=0, columnspan=2,
                  padx=10, pady=10, ipadx=60, ipady=50)
magenta_button.grid(row=0, column=2, columnspan=2,
                    padx=10, pady=10, ipadx=60, ipady=50)
...
tk.Label(game_frame, text="Difficulty: ", font=game_font1,
         bg=game_color).grid(row=2, column=0)
tk.Radiobutton(game_frame, text="Easy", variable=difficulty,
               value="Easy", font=game_font2, bg=game_color, command=set_difficulty).grid(row=2, column=1)
tk.Radiobutton(game_frame, text="Medium", variable=difficulty,
               value="Medium", font=game_font2, bg=game_color, command=set_difficulty).grid(row=2, column=2)
tk.Radiobutton(game_frame, text="Hard", variable=difficulty,
               value="Hard", font=game_font2, bg=game_color, command=set_difficulty).grid(row=2, column=3)
